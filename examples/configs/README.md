...
YAML configuration files for MegaSlide-DiT experiments and the underlying CPU-master training infrastructure.

## MegaSlide-DiT Experiments

```bash
# Smoke test (any GPU, <1 min)
...
## LLM/VLM Training (CPU-Master Infrastructure)

The same CPU-master streaming architecture also supports LLM/VLM training:
...
### MegaSlide-DiT (Video Diffusion)

| Config | Description | Notes |
|:-------|:------------|:------|
| `megaslide_dit_tiny.yaml` | Tiny model for smoke tests | Any GPU, <1 min |
| `megaslide_paper_experiment_tiny.yaml` | Paper experiments at reduced scale | 8+ GB VRAM |
| `megaslide_paper_experiment_256f.yaml` | Full 256-frame paper experiments | 48+ GB VRAM |
| `dense_baseline_64f.yaml` | Dense 3D-DiT baseline | OOMs at 256 frames |
| `swin_baseline_256f.yaml` | Swin-DiT fixed-window baseline | 256 frames, no motion adaptation |

### LLM/VLM (CPU-Master Infrastructure)
