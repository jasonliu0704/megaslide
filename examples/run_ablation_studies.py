...
def ablation_fixed_windows(config):
    ...
    Expected result: VBench-Consist drops from 0.88 to ~0.81
    """
    print("\n" + "=" * 70)
...
def ablation_sync_transfer(trainer):
    ...
    Expected result: Step time increases from 3.1s → 6.8s, MFU drops from 61% → 28%
    """
    print("\n" + "=" * 70)
    ...
    print("  Expected: step time ~3.1s, MFU ~61%")
...
def ablation_gpu_optimizer(config):
    ...
    print(f"\n⚠️  This will OOM on H200 (141 GB) when adding activations (~115 GB)")
...
def main():
    ...
    if args.ablation == "fixed_windows":
        ...
        print("\nExpected impact:")
        print("  VBench-Consist: 0.88 → 0.81 (similar to Swin-DiT)")
        print("  VBench-Align: unchanged (~0.83)")
        ...
        print(f"Average fwd time:  {results['avg_fwd_time']:.2f}s")
        print(f"Average bwd time:  {results['avg_bwd_time']:.2f}s")
        print("\nExpected with SYNC transfers:")
        print("  Step time: 3.1s → 6.8s (2.2x slower)")
        print("  MFU: 61% → 28% (halved)")
