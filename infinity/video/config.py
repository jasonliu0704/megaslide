...
class MegaSlideConfig:
    ...
    # ========== Hybrid Attention (Global Registers) ==========
    num_registers: int = 64
    """Number of global register tokens for hybrid attention."""

    register_interval: int = 4
    """Apply hybrid attention every N layers (1 = every layer)."""

    register_gate_init: float = 0.0
    """Initial gate value (sigmoid applied; 0.0 means gate starts at 0.5)."""

    attention_mode: str = "local"
    """Attention mode: 'local' (pure 3D-DSA), 'hybrid' (registers), 'anchor' (temporal anchors)."""
