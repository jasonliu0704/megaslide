...
class LatentVideoDataset(Dataset):
    ...
    - Separate train/val files via ``val_path``

    Args:
        config: MegaSlideConfig instance.
        path: Optional path to data file (overrides config.path).
        val_path: Optional path to a held-out validation file.
            When provided, a sibling ``LatentVideoDataset`` is accessible
            via ``self.val_dataset`` and the main instance only contains
            the training split.
    """
    ...
    def __init__(self, config: MegaSlideConfig, path: Optional[str] = None,
        ...
        self.val_dataset: Optional["LatentVideoDataset"] = None
        ...
        if val_path and os.path.exists(val_path):
            self.val_dataset = LatentVideoDataset(config, path=val_path)
