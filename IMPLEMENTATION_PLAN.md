...
**Date:** 2026-05-08  
**Paper:** MegaSlide-DiT (105B Video Diffusion Transformer on Single GPU)  
...
3. **Async Double-Buffering** - Overlap weight transfers with compute for 61% MFU (vs 28% without)
...
**Verification:**
- ✅ YAML loading works
- ✅ Memory estimation matches paper Table 2
...
**Performance Expectations (Paper Section 6):**
| Metric | Expected | Measurement |
|--------|----------|-------------|
| Step time | 3.1s | `timing['total']` |
| Forward time | ~40% (1.2s) | `timing['forward']` |
| Backward time | ~60% (1.9s) | `timing['backward']` |
| MFU | 61% | `achieved_flops / 1000 TFLOPs/s` |
...
|-------|--------------|----------------|--------|
| Dense 3D-DiT | 0.78 ± 0.02 | 0.85 ± 0.03 | 64 (max) |
...
**Expected:** VBench-Consist drops from 0.88 to 0.81 (similar to Swin-DiT)
...
**Expected:** Step time increases from 3.1s → 6.8s, MFU drops from 61% → 28%
...
### Full Reproduction (Stretch Goal) 🔜
